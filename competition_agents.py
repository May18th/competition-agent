...
3. 每一段至少有一个「只属于这个项目」的具体名词（产品名 / 场景 / 数据 / 合作方），写不出来就重写这一段。
4. 禁用句式：依托……技术、瞄准……痛点、具有广阔的市场前景、显著的社会价值、为……赋能、打造……生态、全链路、一站式解决方案。
5. 段落不低于 80 字，每段尽量含一个具体数字。
...
def defense_questions_agent(state: CompetitionState) -> CompetitionState:
    ...
    prompt = f"""你是科创赛事答辩专家。请预测评委可能会问的问题，并给出回答要点提示（不是标准答案，是提示该往哪几个方向答）。

项目：{state['idea']}
评委意见：{state['judge_feedback']}

【必须覆盖以下四类尖锐问题，每类至少 2 条】
...
{_tier_hint(state,
"共输出 8 个问题（四类各 2 条），每条附 60～80 字回答要点，简洁点到为止。",
"共输出 12 个问题（四类各 3 条），每条附 150～200 字回答要点，要有具体依据和应对策略。"
)}
"""
    response = llm.invoke([HumanMessage(content=prompt)])
