...
6. 三位专家的 strengths / issues / dimensions[].comment 必须**各自用本视角的专业词汇和关切点**，三份评语严禁出现互相雷同的句子或同一句换个说法：技术视角谈「算法、模型、性能、工程实现」，商业视角谈「客单价、获客成本、毛利、市场空间」，落地视角谈「试点资源、实施周期、团队执行力、可复制性」；谁都不许说另两位已经说过的观点。
7. 中文标点用全角。
"""
    response = llm.invoke([HumanMessage(content=prompt)])
