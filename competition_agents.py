...
3. 每一个自然段都必须同时包含「三要素」，缺一不可：① 至少 1 个「只属于这个项目」的具体用户场景（谁、在什么时刻、做什么、卡在哪一步）；② 至少 1 个可验证数字（真实数据、用户已给数据、或明确标注的占位符【待你填写：xxx】/【示例值，需核实】/测算值）；③ 至少 1 个具体技术名词/参数/功能模块名（如具体算法名、器件型号、框架名、功能模块，不是「先进技术」「智能算法」「深度学习」这类空泛词）。三要素缺一，这一整段必须重写，不许用形容词补。
