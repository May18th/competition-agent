...
def check_proposal_completeness(proposal: str, mode: str = "deep") -> dict:
    ...
    for name, keywords in sections:
        ...
        if not ok:
            missing.append(name)
    # 可交付度指标（替代单纯字数）：占位待补项越少越好、数据支撑点越多越好
    placeholders = len(re.findall(r"【(?:待你填写|示例值|需补充|待补充)[^】]*】", text))
    data_points = len(re.findall(
        r"\d+(?:\.\d+)?\s*(?:%|％|万元|万|亿元|亿|元|次|台|人|家|项|件|年|月|天|小时|km|m|kg)",
        text))
    return {"ok": not missing, "missing": missing, "detail": detail,
            "chars": len(text), "placeholders": placeholders,
...
3. 每一个自然段都必须同时包含「三要素」，缺一不可：① 至少 1 个「只属于这个项目」的具体用户场景（谁、在什么时刻、做什么、卡在哪一步）；② 至少 1 个可验证数字（真实数据、用户已给数据、或明确标注的占位符【待你填写：xxx】/【示例值，需核实】/测算值）；③ 至少 1 个具体技术名词/功能模块名。三要素缺一，这一整段必须重写，不许用形容词补。
4. 禁用句式与空话：依托……技术、瞄准……痛点、具有广阔的市场前景、广阔市场前景、广阔前景、显著的社会效益、显著社会价值、为……赋能、产业赋能、赋能产业、助力、降本增效、提质增效、打造……生态、全链路、一站式解决方案。
...
_SELF_TECH_HIGHLIGHTS = """【如果本项目是「AI 多智能体创作 / 申报书生成 / 竞赛辅导工具」这一类系统，请把下面这些本系统的真实技术亮点写进「项目简介 / 项目概述 / 核心创新点」章节（这些是本系统的真实能力，不是编造，可具体化但不要夸大）】
1. 多智能体并行协作：规则解析、同质化检测、竞品分析、商业模式、风险分析、技术方案、社会价值等多个智能体节点并行生成，再汇总成完整申报书，相比串行流程明显缩短出稿时间。
2. 定向修订闭环：生成后先由模拟评委 + 多专家评审打分挑毛病，再针对评委指出的具体章节做定点修补，而不是整篇推翻重写。
3. 范文库 RAG 检索：内置多赛事真实获奖范文库，按「项目简介 / 创新点 / 商业模式 / 风险分析 / 社会价值」等模块检索注入，让生成内容对标获奖作品的真实写法。
4. 流式输出：生成过程边写边推送，前端实时预览字数与内容，不用干等全文出稿。
5. 图表自动生成：内置 12 种图表（环形 / 柱状 / 雷达 / 象限 / 漏斗 / 甘特 / 架构 / 风险热力 / 堆叠 / 进度环 / 横向排名 / 折线），按内容自动匹配类型，导出 Word / PPT 直接可用。
6. 合规与质量约束：双盲脱敏（隐去院校与指导老师）、去 AI 味、严禁编造假数据、真实行业数据注入。
"""


_SELF_DEFENSE_HINTS = """【如果本项目是「AI 多智能体创作 / 申报书生成 / 竞赛辅导工具」这一类系统，下面 4 个尖锐问题必须出现在 10 个问题里，并各自给出回答框架】
1. 和直接让 ChatGPT / 豆包写一份申报书有什么区别？你的系统凭什么比通用大模型写得更好、更贴比赛评分标准？
2. API 调用成本怎么覆盖？免费额度和收费模式是什么，单次生成大概成本多少、怎么不亏本？
3. 生成内容怎么降重、怎么保证不和其他选手的申报书撞车？有没有查重或去同质化机制？
4. 用户上传的申报书草稿、项目创意涉及隐私，怎么保证数据安全、不被用于训练或泄露？
"""
...
6. 三位专家的 strengths / issues / dimensions[].comment 必须**各自用本视角的专业词汇和关切点**，三份评语严禁出现互相雷同的句子或同一句换个说法：技术视角只挑「硬件选型、实现细节、性能指标、算法/模型」问题，商业视角只挑「市场空间、盈利模式、客单价、获客成本」问题，落地视角只挑「用户画像、使用场景、试点落地、真实用户反馈」问题；谁都不许说另两位已经说过的观点。
7. 中文标点用全角。
"""
    response = llm.invoke([HumanMessage(content=prompt)])
...
【输出格式】每个问题用「## 问题 N：……」作 Markdown 二级标题；紧接着另起一行用「**回答框架**：」加粗开头，再用约 200 字分三点写出：①先摆结论（一句话站住立场）②再给证据（要报出哪个具体数字、哪份测试/试点数据，没有真实数据就明说「此处需补充 XX 实测数据」）③最后堵质疑（提前承认一个短板并给补救说法）。这三点必须落地到本项目，不许写「准备好相关数据即可」这类空框架。不要写成标准答案，不要写成演讲稿，不要照抄申报书原文。
